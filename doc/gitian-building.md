...
*Setup instructions for a gitian build of VintageTimePieceCoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the VintageTimePieceCoin
...
- [Building VintageTimePieceCoin](#building-vintagetimepiececoin)
...
Clone the git repositories for vintagetimepiececoin and gitian,
...
In [doc/release-process.md](release-process.md) in the vintagetimepiececoin repository under 'Fetch and build inputs'.
...
Building VintageTimePieceCoin
----------------

To build VintageTimePieceCoin (for Linux and/or Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the vintagetimepiececoin repository.

Output from `gbuild` will look something like

    Initialized empty Git repository in /home/debian/gitian-builder/inputs/vintagetimepiececoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/vintagetimepiececoin-project/vintagetimepiececoin
...
For example:
```bash
URL=https://github.com/user/vintagetimepiececoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit vintagetimepiececoin=${COMMIT} --url vintagetimepiececoin=${URL} ../vintagetimepiececoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit bitcoin=${COMMIT} --url vintagetimepiececoin=${URL} ../vintagetimepiececoin/contrib/gitian-descriptors/gitian-win.yml
```
...
in `gitian.sigs.VTPC` to your signing machine and do

```bash
    gpg --detach-sign ${VERSION}/${SIGNER}/vintagetimepiececoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/vintagetimepiececoin-build.assert
```
...
[vintagetimepiececoin/gitian.sigs.VTPC](https://github.com/vintagetimepiececoin-project/gitian.sigs.VTPC/) repository, or
