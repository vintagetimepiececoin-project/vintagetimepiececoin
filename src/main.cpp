...
#if defined(NDEBUG)
# error "VintageTimePieceCoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "VintageTimePieceCoin Signed Message:\n";
...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    if (TestNet() && chainActive.Tip()->nHeight + 1 < Params().LastProofOfWorkHeight())
...
void ThreadScriptCheck() {
    RenameThread("vintagetimepiececoin-scriptch");
...
bool AcceptBlock(CBlock& block, CValidationState& state, CDiskBlockPos* dbp)
{
    ...
    if (hash != Params().HashGenesisBlock()) {
        ...
        // VintageTimePieceCoin did not enable this BIP 34
        // TBD
        /*
        if (block.nVersion >= 3)
        {
