...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn)
{
    ...
    //Current active block is last PoW, next block should be PoS
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    //// debug print
    LogPrintf("VintageTimePieceCoinMiner:\n");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("VintageTimePieceCoinMiner : mined block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("VintageTimePieceCoinMiner : ProcessBlock, block not accepted");
    }
...
bool CheckStake(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    //// debug print
    LogPrintf("VintageTimePieceCoinStaker: proof-of-stake found  \n  hash: %s\n  stake: %s\n  target: %s\n", hash.GetHex(), hashStake.GetHex(), hashTarget.GetHex());
    pblock->print();
    ...
    // Found a solution
    {
        LOCK(cs_main);
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("VintageTimePieceCoinStaker : minted block is stale");
        ...
        CValidationState state;
        if (!ProcessBlock(state, NULL, pblock))
            return error("VintageTimePieceCoinStaker : ProcessBlock, block not accepted");
    }

    return true;
}

void VintageTimePieceCoinStaker(CWallet *pwallet)
{
    LogPrintf("VintageTimePieceCoinStaker started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);

    // Make this thread recognisable as the staking thread
    RenameThread("vintagetimepiececoin-staker");
    ...
    try { while (true) {
        if (Params().NetworkID() != CChainParams::REGTEST) {
            ...
            while (vNodes.empty())
            {
                ...
                LogPrintf("VintageTimePieceCoinStaker : Waiting for network online.\n");
        ...
        while (IsInitialBlockDownload()) 
        {
            ...
            LogPrintf("VintageTimePieceCoinStaker : Waiting... Blockchain Downloading.\n");
            MilliSleep(60000);
        }

        while (pwallet->IsLocked())
        {
            LogPrintf("VintageTimePieceCoinStaker : Wallet is locked.\n");
            nLastCoinStakeSearchInterval = 0;
            MilliSleep(1000);
        }

        //Skip if next block is PoW
        while (chainActive.Tip()->nHeight + 1 <= Params().LastProofOfWorkHeight())
        {
            LogPrintf("VintageTimePieceCoinStaker : Chaintip < Last POW.\n");
            MilliSleep(60000);
        }
        ...
        {
            // LogPrintf("VintageTimePieceCoinStaker : Failed to sign the new block.\n");
            MilliSleep(1000);
        }
    } }
    catch (boost::thread_interrupted)
    {
        LogPrintf("VintageTimePieceCoinStaker terminated\n");
        throw;
    }
}

void static VintageTimePieceCoinMiner(CWallet *pwallet)
{
    LogPrintf("VintageTimePieceCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("vintagetimepiececoin-miner");
    ...
    try { while (true) {
        if (Params().NetworkID() != CChainParams::REGTEST) {
            ...
            // on an obsolete chain. In regtest mode we expect to fly solo.
            while (vNodes.empty()){
                LogPrintf("VintageTimePieceCoinMiner : Waiting for network online.\n");
                MilliSleep(1000);
            }
        }
        ...
        {
            // Busy-wait for the download of the blockchain to complete
            LogPrintf("VintageTimePieceCoinMiner : Waiting... Blockchain Downloading.\n");
            MilliSleep(60000);
        }
        ...
        {
            LogPrintf("VintageTimePieceCoinMiner : no more PoW blocks\n");
            return;
        }

        IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

        LogPrintf("Running VintageTimePieceCoinMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("VintageTimePieceCoinMiner terminated\n");
        throw;
    }
}

void GenerateVintageTimePieceCoins(bool fGenerate, CWallet* pwallet, int nThreads)
{
    ...
    minerThreads->create_thread(boost::bind(&VintageTimePieceCoinStaker, pwallet));
    nThreads--;
    ...
    for (int i = 0; i < nThreads; i++) 
        minerThreads->create_thread(boost::bind(&VintageTimePieceCoinMiner, pwallet));
}
